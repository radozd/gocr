...
func boxaCreate(n int) Boxa {
	return Boxa{p: C.boxaCreate(C.int32_t(n))}
}
...
func boxaAddBox(boxa Boxa, box Box, copyflag int) int {
	return int(C.boxaAddBox(boxa.p, box.p, C.int32_t(copyflag)))
}
